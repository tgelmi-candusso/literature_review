...
scrape_gs <- function(term, pages, crawl_delay, ...) {
  ...
  for (n_page in (pages - 1)*10) {  # gs page indexing starts with 0; there are 10 articles per page, see "?start=" param  
    ...
    urls <- html_nodes(wbpage, ".gs_ri") %>% html_node(., ".gs_rt")
    for(j in 1:length(urls)){
      tmp <- urls[j] %>% html_elements("a") %>% html_attr("href", default = NA)
      if(length(tmp) == 0){
        tmp <- " "
      }
      if(j==1){
    ...
    journals <- gsub(".*-(.*),.*", "\\1", authors_years, perl = TRUE)
